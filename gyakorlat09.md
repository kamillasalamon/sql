...
//1. filmek szereplői, neve filmje megjelenés éve, 1960 utáni évek, évszám szerint csökkenő sorrend
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
WHERE m.released>1960
RETURN p.name as nev, m.title as cim,m.released as ev
ORDER BY m.released DESC

//2. filmek címei és szereplői, csak szereplő neve és cím, 1970 utáni VAGY "Love" szó szerepel, cím szerint csökkenő sorrend
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
WHERE m.released>1970 OR m.title CONTAINS 'LOVE' 
RETURN p.name as nev, m.title
ORDER BY m.title DESC

//3. Melyik filmben legtöbb szereplő, csak film címe és szereplők száma
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
...
WITH p.name AS `nev`, COUNT(m.title) AS `db` //új nevek
...
//7.feladat
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
with m.title as nev, count(p.name) as db
return nev, db
